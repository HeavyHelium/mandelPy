...
def mandelbrot(c: complex, max_iters: int = 100) -> int:
    z = c
...
def process_rows(args: Tuple[List[int], np.ndarray, np.ndarray, int]) -> np.ndarray:
...
def mandelbrot_set(xmin: float, xmax: float, ymin: float, ymax: float,
                   width: int, height: int, max_iters: int = 100,
...
max_iters = 256
num_processes = 8
pixels: np.ndarray = mandelbrot_set(xmin, xmax, ymin, ymax, width, height, max_iters, num_processes)
